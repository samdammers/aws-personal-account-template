...
![Personal AWS Account Bootstrap](docs/images/aws-repo-banner.jpg)
...
commit's message says so (`Co-Authored-By: Claude`). Security-sensitive
infrastructure like this deserves extra scrutiny regardless of who or what
wrote it: read through the Terraform and the caveats below before applying
any of this to your own account, rather than trusting the README's word for
what it does.
...
flowchart LR
    ...
    subgraph Auth0["Auth0 tenant"]
        Google["google-oauth2<br/>connection"]
        DB["database connection<br/>(optional, no Google needed)"]
        SAML["AWS IAM Identity Center<br/>SAML app"]
        Guard1["Post-login guard<br/>(email match)"]
        Guard2["Pre-registration guard<br/>(email match)"]
        Google --> SAML
        DB --> SAML
        SAML --> Guard1
        SAML --> Guard2
    ...
    subgraph IIC["IAM Identity Center"]
        Perm["PersonalAdmin<br/>permission set"]
    end

    subgraph Account["Your AWS Account / Organisation"]
        SCP["SCP: region lock +<br/>CloudTrail protection"]
        Trail["CloudTrail -> S3"]
        Cost["Cost Anomaly Detection"]
        Analyzer["IAM Access Analyzer"]
        Contact["Security contact"]
        VPC["Default VPC"]
        SG["Default security group<br/>(locked to zero rules)"]
        VPC --> SG
    end

    You -->|Google login| Google
    You -.->|or email/password| DB
    SAML <-->|SAML federation| IIC
    Perm --> Account
```

The circular-looking arrow between the SAML app and IAM Identity Center is
real, not a diagramming simplification - see
[Bootstrap sequence](#bootstrap-sequence) for why that needs three applies
and one manual step instead of a single `terraform apply`.
...
- **Two login guards restricting that SSO app to you specifically** - the
  ...
  stranger can't get into your AWS account; a second,
  ...
  layer covers and where the uncertainty is.
...
Close to $0/month for personal use. Nothing here runs continuously or scales
with traffic - there's no EC2 instance, no NAT gateway, nothing billed by the
hour:

- AWS Organizations, IAM Identity Center, SCPs, Cost Anomaly Detection, IAM
  Access Analyzer, and the account security contact all have no charge.
- CloudTrail: the first copy of management events delivered to S3 is free;
  you're only paying for storing those logs, a few cents a month at most for
  one account.
- The S3 buckets (CloudTrail logs, the Auth0 branding image, the artifacts
  bucket) and the CloudFront distribution serving that image: negligible at
  personal scale, and CloudFront in particular sits well within the AWS Free
  Tier for the traffic a single login-page image generates.
- Auth0's free tier covers far more monthly active users than one person
  needs.

Realistically the whole stack runs to cents, occasionally low single dollars,
a month, driven by S3/CloudTrail storage rather than anything that grows with
use. Check [CloudTrail pricing](https://aws.amazon.com/cloudtrail/pricing/)
and [S3 pricing](https://aws.amazon.com/s3/pricing/) for current rates if you
want to verify this for your own account.
...
- The one account you're meant to use, at `owner_email`, created
...
- **The region-lock SCP in `scp.tf` is aggressive by design** - it denies
  ...
  organisation (a curated allowlist of IAM/Route53/S3/CloudFront/ACM/billing
...
- **The pre-registration guard (`auth0_action.restrict_to_owner_pre_registration`)
  ...
  signups; whether it fires for a social (Google) connection at all
  ...
  harmless no-op - the post-login guard below is what keeps a
...
- **The optional database connection (`enable_database_connection`) doesn't
  ...
  enforced (this is the connection type Auth0 documents that option
...
- **`aws_default_vpc`/`aws_default_security_group` adopt pre-existing AWS
  ...
  created" the first time even though nothing is being created (the
